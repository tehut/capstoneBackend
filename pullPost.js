...
// takes all fucntions that take callbacks and appends a version  that
// that returns promises instead
  ...
  var options = {
    url: 'http://transparentclassroom.com/api/v1/online_applications.json' + '?created_at=06-01-2017',
    headers: {
  ...
  request.getAsync(options)
    .then((response) => {
      if (response.body.includes('html')) {
          throw new Error("This is an error, a cute picuture of Mika");
      }
...
// the promisifyAll function made getAsync available
